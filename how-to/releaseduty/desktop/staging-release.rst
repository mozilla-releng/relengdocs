...
a matter of rerunning an existing task. However, when the fix is in-tree, it's not clear
that you can reuse and tweak the action task that scheduled release promotion.

1. Copy-paste the input payload in ``task.extra.action.context.input`` of the action
task you can to schedule again.
...
:warning:
   This method by-passes Shipit which can make some release promotion tasks.
